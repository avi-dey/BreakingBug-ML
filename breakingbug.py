...
print("___________________________________________________________")
print ("Mode of the dataset: ",df('data')['age'].mode()) #error 2
print("___________________________________________________________")
...
def impute_categorical_missing_data(wrong_col):
    ...
    for cols in other_missing_cols:
            ...
            imputed_values = IterativeImputer.fit_transform("col_with_missing_values")
    ...
    if len(df_null) > 0:
    df.loc[df_null.index, 'passed'] = classifier.predict(X)
...
for col in Y.columns:
    if Y[col].dtype == 'object':
        ...
        transformed = onehotencoder.fit_transform(Y[[col]])
        # Convert transformed array back to DataFrame
        transformed_df = pd.DataFrame(transformed, columns=onehotencoder.get_feature_names_out([col]))
        # Drop the original column and concatenate the new one-hot encoded columns
        Y = Y.drop(col, axis=1)
        Y = pd.concat([Y, transformed_df], axis=1)
